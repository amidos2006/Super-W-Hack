...
var Global = (function () {
    ...
    Global.matrixReflectX = function (matrix) {
        var result = [];
        for (var x = 0; x < matrix.length; x++) {
            result.push([]);
    ...
    Global.getEmptyTiles = function (tiles) {
        var result = [];
...
var DeathEffect = (function (_super) {
    __extends(DeathEffect, _super);
    function DeathEffect(game, xTile, yTile, gIndex, color, scaleSpeed, alphaSpeed) {
        if (scaleSpeed === void 0) { scaleSpeed = 0.2; }
        if (alphaSpeed === void 0) { alphaSpeed = 0.08; }
        _super.call(this, game, (xTile + 0.5) * Global.TILE_SIZE, (yTile + 0.5) * Global.TILE_SIZE);
        this.alphaSpeed = alphaSpeed;
        this.scaleSpeed = scaleSpeed;
        var graphics = this.game.add.sprite(0, 0, "graphics");
...
/// <reference path="../BaseGameObject.ts"/>
var PlayerEntranceEffect = (function (_super) {
    __extends(PlayerEntranceEffect, _super);
    function PlayerEntranceEffect(game, xTile, yTile, startScale) {
        if (startScale === void 0) { startScale = 5; }
        _super.call(this, game, (xTile + 0.5) * Global.TILE_SIZE, (yTile + 0.5) * Global.TILE_SIZE);
        var graphics = this.game.add.sprite(0, 0, 'graphics');
    ...
    PlayerEntranceEffect.prototype.onAnimationComplete = function () {
        var gameplayState = this.game.state.getCurrentState();
        gameplayState.playerObject = new PlayerObject(this.game, this.getTilePosition().x, this.getTilePosition().y, Global.currentWeapon);
        gameplayState.lastPosition = gameplayState.playerObject.getTilePosition();
        this.game.add.existing(gameplayState.playerObject);
        this.killObject();
    };
    return PlayerEntranceEffect;
}(BaseGameObject));
/// <reference path="../BaseGameObject.ts"/>
var PlayerLeaveEffect = (function (_super) {
    __extends(PlayerLeaveEffect, _super);
    function PlayerLeaveEffect(game, xTile, yTile) {
        _super.call(this, game, (xTile + 0.5) * Global.TILE_SIZE, (yTile + 0.5) * Global.TILE_SIZE);
        var graphics = this.game.add.sprite(0, 0, 'graphics');
        graphics.animations.add("normal", [Global.currentPlayer.graphicsIndex]);
        graphics.animations.play("normal");
    ...
    PlayerLeaveEffect.prototype.onAnimationComplete = function () {
        Global.levelNumber += 1;
        Global.constructLevel(this.game.rnd);
        this.game.state.start("gameplay");
    };
    return PlayerLeaveEffect;
}(BaseGameObject));
/// <reference path="../BaseGameObject.ts"/>
...
var BoxObject = (function (_super) {
    ...
    BoxObject.prototype.killObject = function () {
        this.game.add.existing(new DeathEffect(this.game, this.getTilePosition().x, this.getTilePosition().y, 4, 0xffcc66));
...
var PlayerObject = (function (_super) {
    ...
    PlayerObject.prototype.killObject = function () {
        this.game.add.existing(new DeathEffect(this.game, this.getTilePosition().x, this.getTilePosition().y, Global.currentPlayer.graphicsIndex, 0xffffff));
        _super.prototype.killObject.call(this);
    };
...
var PortalObject = (function (_super) {
    ...
    function PortalObject(game) {
        _super.call(this, game, 0, 0);
        this.sprite = this.game.add.sprite(0, 0, "graphics");
        this.sprite.animations.add("normal", [16, 17], 10, true);
        this.sprite.animations.play("normal");
    ...
    PortalObject.prototype.enterPortal = function () {
        var level = this.game.state.getCurrentState();
        level.playerObject.alpha = 0;
        level.playerObject = null;
        this.game.add.existing(new PlayerLeaveEffect(this.game, this.getTilePosition().x, this.getTilePosition().y));
    };
    return PortalObject;
}(BaseGameObject));
var RoomTypeEnum;
...
var RoomInfoObject = (function () {
    ...
    RoomInfoObject.prototype.constMatrix = function (random) {
        ...
        var shapeSize = random.integerInRange(1, 3);
    ...
    RoomInfoObject.prototype.fixOpenAreas = function () {
        var tileMatrix = this.getMatrix([]);
        var nextTiles = [new Phaser.Point(Math.floor(Global.ROOM_WIDTH / 2), Math.floor(Global.ROOM_HEIGHT / 2))];
        while (nextTiles.length > 0) {
            var currentTile = nextTiles.splice(0, 1)[0];
...
var ANode = (function () {
    ...
    ANode.prototype.setEverything = function (parent, x, y, g, h) {
        this.parent = parent;
        this.x = x;
        this.y = y;
        this.g = g;
...
var Astar = (function () {
    ...
    Astar.prototype.createChild = function (cur, playerPosition, map, sumX, sumY) {
        var child = new ANode();
    ...
    Astar.prototype.dist = function (x1, y1, x2, y2) {
        return Math.sqrt(Math.pow(x1 - x2, 2) + Math.pow(y1 - y2, 2));
    };
    Astar.prototype.search = function (boss, playerPosition, map) {
        var open = [];
        var closed = [];
        var bossPosition = new Phaser.Point(boss.tilePosition.x, boss.tilePosition.y);
        var initial = new ANode();
        initial.x = bossPosition.x;
        initial.y = bossPosition.y;
        var minDist = this.dist(initial.x, initial.y, playerPosition.x, playerPosition.y);
        //find closest point to player
        var aux = 0;
        for (var i = boss.tilePosition.x; i < boss.tilePosition.x + boss.bossWidth; i++) {
            for (var j = boss.tilePosition.y; j < boss.tilePosition.y + boss.bossHeight; j++) {
        ...
        var dir = [
            ...
            new Phaser.Point(0, -1)];
        while (open.length > 0) {
            open.sort(function (a, b) { return (a.f() - b.f()) * (-1); });
            cur = open.pop();
            ...
            for (var i = 0; i < dir.length; i++) {
                if (map[cur.x + dir[i].x][cur.y + dir[i].y] != TileTypeEnum.Wall &&
                    ...
                    var child = this.createChild(cur, playerPosition, map, dir[i].x, dir[i].y);
    ...
    Astar.prototype.colide = function (cur, boss, player) {
        for (var i = cur.x; i <= cur.x + boss.width; i++) {
            for (var j = cur.y; j <= cur.y + boss.height; j++) {
    ...
    Astar.prototype.appearInList = function (list, child) {
        var has = false;
...
var Boss = (function (_super) {
    ...
    function Boss(game, xTile, yTile) {
        ...
        this.dashSpeed = 2;
        this.probDirection = [25, 25, 25, 25];
    ...
    Boss.prototype.colide = function (pos, map) {
        for (var i = pos.x; i < pos.x + this.bossWidth; i++) {
            for (var j = pos.y; j < pos.y + this.bossHeight; j++) {
    ...
    Boss.prototype.selectNewDirection = function () {
        var dir = this.game.rnd.integerInRange(0, 100);
    ...
    Boss.prototype.probCurDirectionNearZero = function () {
    ...
    Boss.prototype.moveRandom = function (map) {
        var previousPos = new Phaser.Point(this.tilePosition.x, this.tilePosition.y);
    ...
    Boss.prototype.moveChase = function (playerPosition, map) {
        var nextMove = new Astar().search(this, playerPosition, map);
        this.tilePosition.x += nextMove.x;
        this.tilePosition.y += nextMove.y;
        this.x = this.toTile(this.tilePosition.x);
        this.y = this.toTile(this.tilePosition.y);
    };
    Boss.prototype.specialSpawnEnemy = function (playerPosition, map) {
        //add enemies
        var level = this.game.state.getCurrentState();
        //level.add(enemy);
        var enx = 0, eny = 0;
        var auxMap = new Array(map.length);
        for (var i = 0; i < auxMap.length; i++) {
            auxMap[i] = new Array(map[0].length);
            for (var j = 0; j < auxMap.length; j++) {
        ...
        for (var i = this.tilePosition.x; i <= this.tilePosition.x + this.bossWidth; i++) {
            for (var j = this.tilePosition.y; j <= this.tilePosition.y + this.bossHeight; j++) {
                auxMap[i][j] = 1;
            }
        }
        for (var i = playerPosition.x > 0 ? playerPosition.x - 1 : playerPosition.x; i <= (playerPosition.x < map.length - 1 ? playerPosition.x - 1 : playerPosition.x); i++) {
            for (var j = playerPosition.y > 0 ? playerPosition.y - 1 : playerPosition.y; j <= (playerPosition.y < map[0].length - 1 ? playerPosition.y - 1 : playerPosition.y); j++) {
                auxMap[i][j] = 1;
            }
        }
    ...
    Boss.prototype.createRandomEnemy = function (playerPosition, map) {
        var enX = 0, enY = 0;
        var random = new RandomEnemyObject(this.game, enX, enY, 1, 0, new Phaser.Point(0, 0));
        return random;
    };
    /**
...
var EnemyObject = (function (_super) {
    ...
    EnemyObject.prototype.killObject = function () {
        this.game.add.existing(new DeathEffect(this.game, this.getTilePosition().x, this.getTilePosition().y, this.enemySpriteIndex, 0xcc6668));
        _super.prototype.killObject.call(this);
    };
...
/// <reference path="../Enemy/EnemyObject.ts"/>
var BlobEnemyObject = (function (_super) {
    __extends(BlobEnemyObject, _super);
    function BlobEnemyObject(game, x, y, health, numberOfCannons, cannonDirection1, firstPlayerPosition) {
        _super.call(this, game, x, y, health, numberOfCannons, cannonDirection1);
        this.trailList = [];
    ...
    BlobEnemyObject.prototype.dropTrail = function () {
        var trail;
        console.log(this.thereIsATrailInThisPosition(this.getTilePosition()));
        if (!this.thereIsATrailInThisPosition(this.getTilePosition())) {
            trail = new TrailObject(this.game, this.getTilePosition().x, this.getTilePosition().y);
            this.trailList.push(trail);
        }
    };
    BlobEnemyObject.prototype.thereIsATrailInThisPosition = function (position) {
    ...
    BlobEnemyObject.prototype.enemyMove = function (enemyDirection, tileMatrix) {
        var direction = new Phaser.Point((enemyDirection.x - this.firstPlayerPosition.x), (enemyDirection.y - this.firstPlayerPosition.y));
    ...
    return BlobEnemyObject;
}(EnemyObject));
var CannonObject = (function () {
    function CannonObject(cannonDirection) {
...
var EnemyFactory = (function () {
    ...
    EnemyFactory.getEnemey = function (game, x, y, random, player) {
        var enemyObject;
        var randomType = 0; //random.integerInRange(0, 2);
        if (randomType == 0) {
            enemyObject = new BlobEnemyObject(game, x, y, random.integerInRange(1, 3), 0, player.getTilePosition());
        }
...
var RandomEnemyObject = (function (_super) {
    ...
    RandomEnemyObject.prototype.getRandomFreeDirection = function (enemyDirection, tileMap) {
        ...
        for (var i = 0; i < this.directions.length; i++) {
            if (this.updateEnemy(this.directions[i], tileMap)) {
                if (!enemyDirection.equals(this.directions[i]))
                    dirs.push(this.directions[i]);
            }
        }
        if (dirs.length > 0) {
    ...
    RandomEnemyObject.prototype.takeDamage = function (damage) {
        var result = _super.prototype.takeDamage.call(this, damage);
        if (!result && damage > 0) {
...
var TrailObject = (function (_super) {
    __extends(TrailObject, _super);
    function TrailObject(game, x, y) {
        _super.call(this, game, x, y);
    }
    TrailObject.prototype.printTrail = function () {
        ...
        if (this.repeat) {
            if (faceDirection.x == 1) {
                for (var i = (topLeft.y < 0 ? 0 : topLeft.y); i - topLeft.y < pathRightDir.length; i++) {
                    for (var j = topLeft.x; j < result[0].length; j++) {
...
var GameplayState = (function (_super) {
    ...
    GameplayState.prototype.createCurrentRoom = function (room) {
        ...
        this.highlightTiles = [];
        for (var i = 0; i < 4 * (Global.ROOM_WIDTH + Global.ROOM_HEIGHT); i++) {
        ...
        this.enemyObjects = [];
        var numOfEnemies = 1; //Global.enemyNumbers.getNumber(this.rnd, Global.levelNumber);
        if (room.cleared || room.roomType == RoomTypeEnum.None || room.roomType == RoomTypeEnum.Boss) {
            numOfEnemies = 0;
        }
        var normalTiles = room.getMatrix(this.enemyObjects);
        for (var i = 0; i < numOfEnemies; i++) {
            var tempEnemy = Global.enemyTypes.getEnemy(this.game, normalTiles, Global.currentWeapon.getWeaponPositions(new Phaser.Point(Math.floor(Global.ROOM_WIDTH / 2), Math.floor(Global.ROOM_HEIGHT / 2)), new Phaser.Point(0, -1), normalTiles), this.playerObject);
        ...
        if (Global.previousDirection.getMagnitude() == 0) {
            ...
            this.game.add.existing(new PlayerEntranceEffect(this.game, Math.floor(Global.ROOM_WIDTH / 2), Math.floor(Global.ROOM_HEIGHT / 2) + shift, 8));
        }
        else {
    ...
    GameplayState.prototype.addEnemy = function (enemy) {
...
var LoadingState = (function (_super) {
    ...
    LoadingState.prototype.preload = function () {
        ...
        var text = this.game.add.text(this.game.width / 2, this.game.height / 2, "loading", style);
        text.anchor.set(0.5, 0.5);
        this.game.add.existing(new WhiteLayout(this.game, 10, 10, this.game.width - 20, this.game.height - 20));
...
var EnemyNumbers = (function () {
    function EnemyNumbers(game) {
        this.probabilities = [];
        var probString = game.cache.getText("enemyNumbers").split("\n");
        for (var i = 0; i < probString.length; i++) {
            this.probabilities.push([]);
            var values = probString[i].split(",");
            for (var j = 0; j < values.length; j++) {
                this.probabilities[i].push(parseInt(values[j]) / 100.0);
            }
        }
    }
    EnemyNumbers.prototype.getNumber = function (random, levelNumber) {
        var cdf = [this.probabilities[levelNumber][0]];
        for (var i = 1; i < this.probabilities[levelNumber].length; i++) {
            cdf.push(cdf[i - 1] + this.probabilities[levelNumber][1]);
        }
        var value = random.realInRange(0, 1);
...
var EnemyNames;
(function (EnemyNames) {
    EnemyNames[EnemyNames["Random"] = 0] = "Random";
    EnemyNames[EnemyNames["Chaser"] = 1] = "Chaser";
    EnemyNames[EnemyNames["Patrol"] = 2] = "Patrol";
})(EnemyNames || (EnemyNames = {}));
var EnemyTypes = (function () {
    function EnemyTypes(game) {
        this.typeProbabilities = [];
        var probString = game.cache.getText("enemyTypes").split("\n");
        for (var i = 0; i < probString.length; i++) {
    ...
    EnemyTypes.prototype.initNewRoom = function (levelNumber, lastDirection) {
    ...
    EnemyTypes.prototype.getIndex = function (random, prob) {
        var cdf = [this.typeProbabilities[this.levelNumber][0]];
    ...
    EnemyTypes.prototype.getFarAwayTiles = function (map) {
        var locations = Global.getEmptyTiles(map);
        var playerLocation = new Phaser.Point(Math.floor((this.lastDirection.x + 1) * Global.ROOM_WIDTH / 2)
    ...
    EnemyTypes.prototype.createRandom = function (game, map, damage, distances, player) {
        var locations = this.getFarAwayTiles(map);
        var empty = locations[game.rnd.integerInRange(0, locations.length - 1)];
        var health = this.getIndex(game.rnd, this.healthProbabilites) + 1;
        var maxHealth = true;
        ...
        return new BlobEnemyObject(game, empty.x, empty.y, health, 0, new Phaser.Point(), player.getTilePosition());
    };
    EnemyTypes.prototype.createChaser = function (game, map, damage, distances) {
        var locations = this.getFarAwayTiles(map);
        var empty = locations[game.rnd.integerInRange(0, locations.length - 1)];
        var health = this.getIndex(game.rnd, this.healthProbabilites) + 1;
        var maxHealth = true;
    ...
    EnemyTypes.prototype.getBestPatrolPositions = function (random, map, refrenceValue) {
        var positions = [];
        var playerLocation = new Phaser.Point(Math.floor((this.lastDirection.x + 1) * Global.ROOM_WIDTH / 2)
            - Math.floor((this.lastDirection.x + 1) / 2), Math.floor((this.lastDirection.y + 1) * Global.ROOM_HEIGHT / 2)
            - Math.floor((this.lastDirection.y + 1) / 2));
    ...
    EnemyTypes.prototype.createPatrol = function (game, map, damage, distances) {
        var position = new Phaser.Point();
        var health = this.getIndex(game.rnd, this.healthProbabilites) + 1;
        var cannonDirection = this.patrols.splice(game.rnd.integerInRange(0, this.patrols.length - 1), 1)[0];
        var moveDirection = new Phaser.Point(cannonDirection.y, cannonDirection.x);
        if (Math.random() < 0.5) {
            moveDirection.x *= -1;
            moveDirection.y *= -1;
        }
    ...
    EnemyTypes.prototype.getEnemy = function (game, map, damageMatrix, player) {
        var damageValue = 0;
        var distances = [];
        for (var x = 0; x < Global.ROOM_WIDTH; x++) {
            for (var y = 0; y < Global.ROOM_HEIGHT; y++) {
                if (damageMatrix[x][y] > 0) {
                    ...
                    var d = Math.abs(x - Math.floor(Global.ROOM_WIDTH / 2)) +
        ...
        switch (currentClassIndex) {
            case 0:
                enemy = this.createRandom(game, map, damageValue, distances, player);
                break;
            case 1:
                enemy = this.createRandom(game, map, damageValue, distances, player);
                break;
